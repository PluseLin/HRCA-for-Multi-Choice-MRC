...
class DreamProcessor(DataProcessor):
    ...
    def _convert_speaker(self,passage:str):
        if(passage.startswith("M")):
            passage="man"+passage[1:]
        elif(passage.startswith("W") or passage.startswith("F")):
            passage="woman"+passage[1:]
        return passage
                ...
                with open([data_dir + "/" + "train.json", data_dir + "/"  + "dev.json",
                    ...
                    for i in range(len(data)):
                        for j in range(len(data[i][1])):
                            for idx,sentence in enumerate(data[i][0]):
                                data[i][0][idx]=self._convert_speaker(sentence)
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(tqdm.tqdm(examples, desc="convert examples to features")):
        ...
        for ending_idx, (context, ending) in enumerate(zip(example.contexts, example.endings)):
            ...
            t_pqo_pos=[
                1+t_c_len-1,
                1+t_c_len+t_q_len+sep_tok_len-1,
                1 + t_c_len + sep_tok_len + t_q_len + t_o_len - 1
            ]
            pqo_pos.append(t_pqo_pos)

...
def train(args, train_dataset, model, tokenizer):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            #if isinstance(model,AlbertHRCA):
            if args.model_type=="albert":
                inputs["pqo_pos"]=batch[4]
    ...
    logger.info("best dev acc:{},best dev loss{}".format(best_dev_acc,best_dev_loss))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test=False):
    ...
    # logger.info("train_cache_file=%s", args.train_cache_file)
    # logger.info("os.path.exists(args.train_cache_file)=={}".format(os.path.exists(args.train_cache_file)))
    ...
    all_pqo_pos=torch.tensor([f.pqo_pos for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_pqo_pos)
